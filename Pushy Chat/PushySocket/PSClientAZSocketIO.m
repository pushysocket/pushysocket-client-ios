...
- (BOOL)loginWithName:(NSString *)name {
    ...
    NSError *anError = nil;
    BOOL success = [_socketIO emit:@"login" args:@{@"id":self.clientId ? self.clientId : @"", @"name":name ? name : @"", @"device":self.pushRegistrationToken ? self.pushRegistrationToken : @""} error:&anError];
